    ...
    def __init__(self, data_name, mode, transformer):
        ...
        if isinstance (transformer, str):
            if transformer == "LN":
                ...
                self.transformer =Dataset.transformer_none()
            else:
...
def get_data_model(config, device):
    ...
    elif config.model.name=="LinearNO":
        from module.LinearAttentionNeuralOperator import LinearAttentionNeuralOperator
        model=LinearAttentionNeuralOperator(space_dim=x_dim,n_layers=config.model.n_block,n_hidden=config.model.n_dim,n_head=config.model.n_head,
                       mlp_layer=config.model.n_layer,fun_dim=f_dim,out_dim=y_dim).to(device)
    else:
