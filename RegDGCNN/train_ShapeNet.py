...
from preprocessors_ShapeNet.Dataset import VTKDataset, SATO_Dataset, sato_collate_fn
...
def train_and_evaluate(args, device):
    ...
    exp_dir = os.path.join("experiments_ShapeNet", args.exp_name)
    ...
    for epoch in range(args.training.epochs):
        ...
        if (epoch + 1) % 10 == 0 or epoch == args.training.epochs - 1:
            ...
            plt.savefig(
                os.path.join("experiments_ShapeNet", args.exp_name, "training_progress.png")
            )
            plt.close()
    ...
    test_model(
        ...
        criterion,
        device,
        os.path.join("experiments_ShapeNet", args.exp_name),
        args,
    )
