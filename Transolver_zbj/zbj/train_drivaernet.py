...
from preprocessors_drivaernet.data_loader import get_dataloaders, PRESSURE_MEAN, PRESSURE_STD
...
def train_and_evaluate(rank, world_size, args):
    ...
    train_dataloader, val_dataloader, test_dataloader = get_dataloaders(
        args.dataload.dataset_path, args.dataload.subset_dir, args.dataload.num_points,
        args.training.batch_size, world_size, rank, args.dataload.cache_dir,
...
def test_model(model, test_dataloader, criterion, local_rank, exp_dir):
    ...
    with torch.no_grad():
        for batch in tqdm(test_dataloader, desc="[Testing]"):
            ...
            pred_den = normalizers["surface_anchor_pressure"].denormalize(pred_s_pressure)
            targ_den = normalizers["surface_anchor_pressure"].denormalize(targets_s_pressure)
