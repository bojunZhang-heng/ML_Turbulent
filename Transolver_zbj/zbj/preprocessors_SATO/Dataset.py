...
class VTKDataset():
    ...
    def get_data_dict(self, directory):
        ...
        for file_path in SurfacePressure_file_paths:
            ...
            Surface_pres_points = np.load(os.path.join(directory, 'SurfacePressure', 'points_s', f'points_{index}.npy'))
            Surface_pressure = np.load(os.path.join(directory, 'SurfacePressure', 'pressure_s', f'pressure_{index}.npy'))
            Surface_wss_points = np.load(os.path.join(directory, 'SurfaceWSS', 'points_s', f'points_{index}.npy'))
            Surface_wss = np.load(os.path.join(directory, 'SurfaceWSS', 'wss_s', f'wss_{index}.npy'))
