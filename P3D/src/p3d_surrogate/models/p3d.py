...
class ConditionedEncoder3DBlock(nn.Module):
    ...
    def forward(self, x, embedding):
        ...
        # 给样本条件embedding施加一个线性层Linear()，动态生成scale和shift
        ...
        # 条件调制：利用外部条件embedding对input进行缩放和平移
        # x_c = x_c * (1 + s_c) + b_c 运算发生在第二个维度 通道C
...
class ConditionedEncoder3D(nn.Module):
    ...
    def forward(self, x, embedding):
        ...
        # 输入特征, x.shape=[1, 4, 16, 16, 16]
        # conv3d编码, x.shape = [1, 32, 16, 16, 16]
        x = self.feature_embed(x)

        # 保留下采样之前的高分辨率特征 res_list[0].shape=[1, 32, 16, 16, 16]
        res_list = [x]

        # 第一次下采样, Conv3d卷积层
        # x.shape = [1, 32, 16, 16, 16] 
        # x.shape = [1, 32, 8, 8, 8]
        x = self.downsampling_layers[0](x)

        # 连续执行ConditionEncoder3DBlock 条件编码块，保存每次的特征，再进行下采样
        # 每个中间分辨率阶段： 重复执行repetions次ConditionEncoder3DBlock
        # 条件编码就是在根据样本条件编码embedding创建缩放和平移，
        # 在 x 经过Conv3D后，在特征 x 中加入缩放和平移
        # 再每次返回时保留之前的特征-残差链接
...
def window_partition(x, window_size):
    '''
        将整个三维空间划分成许多局部窗口，将全局注意力(N^2)转换成局部注意力(window_size^2)
        ...
        用 permute 调整维度顺序, 确保用reshape调整矩阵形状时窗口编号和窗口内部坐标不会混杂在一起
        (B,
            H_blocks, W_blocks, D_blocks,
            H_local,  W_local,  D_local,
        C)

        用 reshape 单纯调整矩阵形状
        (window_number, window_size^3, channel)
...
def P3D_S(channel_size: int,
    ...
    # 定义一个字典，包含模型位置和所需传递参数
    model = {
        'target': 'p3d_surrogate.models.p3d.P3DBackbone',
        'params': {
            'hidden_size': 64,
...
class WrapperModel3D(ModelMixin, ConfigMixin):
    ...
    def encode(self,
        ...
        # ConditionedEncoder3D： 条件三维卷积块 + scale-shift调制 + 残差连接
        # len(residuals)=3, 一层下采样残差，一层条件编码残差，一层下采样残差
        residuals = self.encoder(x, emb)

        # 取出最后一个，最深层的特征 state.shape = [1, 64, 4, 4, 4]
        state = residuals[-1]

        # 进入P3DBackbone
        # latent.shape = [1, 256, 1, 1, 1]
        # len(emb_backbone) = 3, 两层encoder➕一层latent
        # len(residuals_backbone) = 2
        latent, emb_backbone, residuals_backbone = (
            self.model_impl.encode(state, timestep, class_labels, pde_parameters))
    ...
    def decode(self, x, embedding, residuals):
        ...
        # 外层编码器深层特征➕P3Dbackbone解码特征, 跨模块残差融合
    ...
    def forward(
        ...
        # len(encoded.hidden_states) = 6
        # hidden_states=residual(3)+ residuals_backbone(2) + [latent](1)
        # len(encoded.embedding) = 4
        # embedding=[emb](1) + emb_backbone(3)
        encoded = self.encode(hidden_states, timestep, class_labels, pde_parameters)
...
class WindowAttention3D(nn.Module):
    ...
    def forward(self, x, attn_mask=None):
        ...
        if self.attn_type == 'v1':
            ...
            # 创建一个缩放因子，最大值是100，最小值>0, e^4.6052=100
            # self.logit_scale 是可学习的参数
        ...
        # 在注意力矩阵上加入相对位置(网格索引差，并不是真实的空间坐标信息)
...
class P3DStage(nn.Module):
    ...
    def maybe_pad(self, hidden_states, height, width, depth):
        ...
        # pad()是从最后一个维度开始解释
        # hidden_states.shaoe = [B, H, W, D, C]
        # pad_values.shape = [C_left, C_right, D_L, D_R, W_L, W_R, H_L,H_R]
    ...
    def forward(self,
        ...
        for n, block in enumerate(self.blocks):
            ...
            # maybe_pad() 确保input能够被window_size整除，无法整除时，空间方向上右侧补零
...
class P3DBackbone(nn.Module):
    ...
    def __init__(
        ...
        for i in range(self.num_encoder_layers + 1):
            ...
            # 等价于 self.t_embedder_0 = TimestepEmbedder(64)
            # 这里面的timestep并不等同于input的物理时间，可以是同一个物理实现的input在神经网络中的不同阶段的时间嵌入模块
    ...
    def encode(self, x, t, class_labels, pde_parameters):
        ...
        # 将时间、pde参数、label这三类信息融合成一个条件向量
        for i in range(self.num_encoder_layers + 1):
            # 时间和pde参数是利用傅立叶变换由标量转换成频率
            # label是另一种方法
        ...
        for i, c in enumerate(emb_list[:-1]):
            # encoder 
            # x.shape = [1, 64, 4, 4, 4].  c.shape = [1, 64]
            out_enc_level = self.__getattr__(f"encoder_level_{i}")(x, c)

            # out_enc_level.shape = [1, 64, 4, 4, 4]
        ...
        # x.shape = [1, 256, 1, 1, 1]
        # len(emb_list) = 3
        # len(residuals_list) = 2， 只存储前两层编码的残差，第三层latent残差就是 x, 不存储
        return x, emb_list, residuals_list

    def decode(self, x, embeddings, residuals):

        # residuals = [r0, r1]
        # residuals[1:], 从下标1开始一直取到列表末尾
        # tmp = residuals[::-1] 将列表倒序排列, tmp = [r1, r0], 从较深的编码器特征->较浅的编码器特征

        # embedding[1:-1], 从下标1开始，取到倒数第一个元素之前，不包括最后一个

        # zip(), 得到residuals和embedding的配对关系, 确保每次循环都是同一层的residual和embedding
        ...
        # 上采样
        # x.shape = [1, 128, 2, 2, 2] -> [1, 32, 4, 4, 4]
