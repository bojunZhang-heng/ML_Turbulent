...
cmap_nonlinear_alt = sns.color_palette("rocket", as_cmap=True)  # used for vape-c3
...
def render_vape_3d(volume, cmap, height, width, time=0.1, vmin=None, vmax=None,
                   background=(255, 255, 255, 255), camera_angle=None,
                   highlight_box=None,
                   highlight_box_color=None,
                   highlight_box_linewidth=None):
    if vmin is None:
        vmin, _ = symmetric_min_max(volume)
    if vmax is None:
        _, vmax = symmetric_min_max(volume)

    # vape4d expects background as float [0, 1]; normalise from uint8 if needed
    bg = tuple(c / 255.0 for c in background) if max(background) > 1.0 else background
    ...
    img = render(
        ...
        highlight_box_linewidth=highlight_box_linewidth,
    )
    # gamma-correct RGB channels only; leave alpha linear
    img_float = img / 255.0
    rgb_gamma = np.power(np.clip(img_float[:, :, :3], 0.0, 1.0), 2.4)
    alpha = img_float[:, :, 3:4]
    return np.concatenate([rgb_gamma, alpha], axis=2)  # (H, W, 4) float
...
def render_trajectory(
    ...
    fig.savefig(os.path.join(output_path,"sim_%04d.pdf" % sim_id), bbox_inches="tight", pad_inches=0.5)
    plt.close(fig)
