...
def train_one_epoch(model, train_dataloader, optimizer, criterion, local_rank):
    ...
    for batch in tqdm(train_dataloader, desc="[Training]"):
        ...
        loss = criterion(pred_wss, targets_wss)
...
def test_model(model, test_dataloader, criterion, local_rank, exp_dir):
    ...
    with torch.no_grad():
        for batch in tqdm(test_dataloader, desc="[Testing]"):
            ...
            mse_loss = criterion(pred_wss, targets_wss)
            total_loss += mse_loss.item()
            pred_den = normalizers["surface_anchor_wallshearstress"].denormalize(pred_wss)
            targ_den = normalizers["surface_anchor_wallshearstress"].denormalize(targets_wss)
