...
def train(model_name, device, model, train_loader, optimizer, scheduler, reg=1, pos_norm=0, norm_norm=0, out_norm=1, pos_mean=None, pos_std=None, norm_mean=None, norm_std=None, out_mean=None, out_std=None, full=False):
    model.train()

    criterion_func = nn.MSELoss(reduction='none')
    losses_mse = []
    for x, y, pos, geom, seg_matrix, inverse_seg_matrix in train_loader:
        x = x.to(device)    # [B, N, 7]
        ...
        if pos_norm:
            ...
            x[:, :, :3] = pos
        y = y.to(device)    # [B, N, 6]
        geom = geom.to(device)    # [B, 3]
        seg_matrix = seg_matrix.to(device)    # [B, S, N]
        inverse_seg_matrix = inverse_seg_matrix.to(device)    # [B, S, N]
        optimizer.zero_grad()
        
        if model_name == 'Transolver_plus':
            out = model((x, pos, geom))
        elif model_name[:14] == 'Transolver_seg':
            out = model((x, pos, geom, seg_matrix, inverse_seg_matrix))
        else:
...
@torch.no_grad()
def test(model_name, device, model, test_loader, pos_norm=0, norm_norm=1, out_norm=1, pos_mean=None, pos_std=None, norm_mean=None, norm_std=None, out_mean=None, out_std=None, full=False):
    model.eval()
    ...
    for x, y, pos, geom, seg_matrix, inverse_seg_matrix in test_loader:
        # 计时
        x = x.to(device)
        pos = pos.to(device)
        if pos_norm:
...
def main(model_name, device, train_loader, val_loader, Net, hparams, path, reg=1, val_iter=1, pos_norm=0, out_norm=1, norm_norm=0, pos_mean=None, pos_std=None, out_mean=None, out_std=None, norm_mean=None, norm_std=None, full=False):
    model = Net.to(device)
    ...
    for epoch in pbar_train:
        loss_mse = train(model_name, device, model, train_loader, optimizer, lr_scheduler, reg=reg, pos_norm=pos_norm, out_norm=out_norm, norm_norm=norm_norm, pos_mean=pos_mean, pos_std=pos_std, out_mean=out_mean, out_std=out_std, norm_mean=norm_mean, norm_std=norm_std, full=full)
        train_loss = loss_mse

        if val_iter is not None and (epoch == hparams['nb_epochs'] - 1 or epoch % val_iter == 0):
            loss_mse, loss_l2re = test(model_name, device, model, val_loader, pos_norm=pos_norm, out_norm=out_norm, norm_norm=norm_norm, pos_mean=pos_mean, pos_std=pos_std, out_mean=out_mean, out_std=out_std, norm_mean=norm_mean, norm_std=norm_std, full=full)
