...
class GeometrySTLDataset(Dataset):
    """
    Dataset class for loading and preprocessing Geometry CAD data from DrivAerNet++ STL files.

    This dataset handles loading .stl files,
    ...
    def __init__(self, root_dir: str, num_points: int, preprocess=False, cache_dir=None):
        """
        Initializes the GeometrySTLDataset instance.

        Args:
            root_dir: Directory containing the STL files for the car surface meshes.
        ...
        self.stl_files = [os.path.join(root_dir, f) for f in os.listdir(root_dir) if f.endswith('.stl')]
    ...
    def _get_cache_path(self, stl_file_path):
        """Get the corresponding .npz file path for a given .stl file."""
        base_name = os.path.basename(stl_file_path).replace('.stl', '.npz')
        return os.path.join(self.cache_dir, base_name)

    def _save_to_cache(self, cache_path, point_cloud):
        """Save preprocessed point cloud and pressure data into an npz file."""
        np.savez_compressed(cache_path, points=point_cloud.points)

    def _load_from_cache(self, cache_path):
        """Load preprocessed point cloud from an npz file."""
        data = np.load(cache_path)
        point_cloud = pv.PolyData(data['points'])
        return point_cloud

    def sample_point_cloud_from_stl(self, mesh, n_points=5000):
        """
        Sample n_points from the surface mesh.
        ...
        return pv.PolyData(sampled_points)

    def __getitem__(self, idx):
        stl_file_path = self.stl_files[idx]
        cache_path = self._get_cache_path(stl_file_path)

        # Check if the data is already cached
        if os.path.exists(cache_path):
            logging.info(f"Loading cached data from {cache_path}")
            point_cloud = self._load_from_cache(cache_path)
        else:
            if self.preprocess:
                logging.info(f"Preprocessing and caching data for {stl_file_path}")
                try:
                    mesh = pv.read(stl_file_path)
                except Exception as e:
                    logging.error(f"Failed to load STL file: {stl_file_path}. Error: {e}")
                    return None, None  # Skip the file and return None

                point_cloud = self.sample_point_cloud_from_stl(mesh, self.num_points)

                # Cache the sampled data to a new file
                self._save_to_cache(cache_path, point_cloud)
            else:
                logging.error(f"Cache file not found for {stl_file_path} and preprocessing is disabled.")
