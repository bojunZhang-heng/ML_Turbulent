...
#        num_surf_blocks: int = 6,  # 6 modality-specific self-attention blocks
#        radius: float = 0.25,  # radius for supernode pooling
        ...
        for block in args.blocks:
            ...
            self.blocks.append(block_ctor(dim=args.dim, num_heads=args.num_heads, patch_size=20, shift=2, dropout=0.1))
        ...
        self.surface_decoder = nn.Linear(args.dim, args.output_dim_surface)
        self.volume_decoder = nn.Linear(args.dim, args.output_dim_volume)
    ...
    def forward(
        self,
        volume_order: torch.Tensor,
        volume_inverse: torch.Tensor,
        surface_order: torch.Tensor,
        surface_inverse: torch.Tensor,
        # geometry
        ...
        for block in self.geometry_blocks:
            x = block(x, geometry=True, attn_kwargs=geometry_attn_kwargs)
        geometry_encoding = x
        ...
        # surface blocks
        x = x_surface
        _, H, _ = surface_order.shape
        for ii, block in enumerate(self.surface_blocks):

            serialization_type = int(ii % H)
            x = block(
                x,
                surface_order[:, serialization_type, :],
                surface_inverse[:, serialization_type, :],
        ...
        for ii, block in enumerate(self.volume_blocks):
            ...
            x = block(
                x,
                volume_order[:, serialization_type, :],
                volume_inverse[:, serialization_type, :],
