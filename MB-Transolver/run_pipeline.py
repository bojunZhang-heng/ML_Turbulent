...
Pipeline script for the Multi-Branc Transolver
pressure field and volumetric field prediction project.
...
def parse_args():
    ...
    # Data settings
    parser.add_argument('--Cdataset_path', type=str, required=True, help='Path to dataset')
    parser.add_argument('--Pdataset_path', type=str, required=True, help='Path to dataset')
    parser.add_argument('--Wdataset_path', type=str, required=True, help='Path to dataset')
    parser.add_argument('--Vdataset_path', type=str, required=True, help='Path to dataset')
...
def preprocess_data(args):
    ...
    Pcache_dir = args.Pcache_dir or os.path.join(args.Pdataset_path, "processed_data")
    os.makedirs(Pcache_dir, exist_ok=True)

    Wcache_dir = args.Wcache_dir or os.path.join(args.Wdataset_path, "processed_data")
    os.makedirs(Wcache_dir, exist_ok=True)

    Ccache_dir = args.Ccache_dir or os.path.join(args.Cdataset_path, "processed_data")
    os.makedirs(Ccache_dir, exist_ok=True)

    Vcache_dir = args.Vcache_dir or os.path.join(args.Vdataset_path, "processed_data")
...
def train_model(args):
    ...
    if args.Pcache_dir:
        cmd.extend(["--Pcache_dir", args.Pcache_dir])
    if args.Wcache_dir:
        cmd.extend(["--Wcache_dir", args.Wcache_dir])
    if args.Ccache_dir:
        cmd.extend(["--Ccache_dir", args.Ccache_dir])
    if args.Vcache_dir:
        cmd.extend(["--Vcache_dir", args.Vcache_dir])
