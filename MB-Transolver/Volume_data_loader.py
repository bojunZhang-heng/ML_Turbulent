...
class VolumeDataset(Dataset):
    """
    Dataset class for loading and preprocessing volumetric data from DrivAerNet++ VTK files.

    This dataset handles loading volume meshes with pressure field data,
    ...
    def __init__(self, root_dir: str, num_points: int, preprocess=False, cache_dir=None):
        """
        Initializes the VolumeDataset instance.
    ...
    def _save_to_cache(self, cache_path, point_cloud, pressures, wss, vel):
        """Save preprocessed point cloud and pressure data into an npz file."""
        np.savez_compressed(cache_path, points=point_cloud.points, pressures=pressures, wss=wss, vel=vel)
    ...
    def _load_from_cache(self, cache_path):
        ...
        wss = data['wss']
        vel = data['vel']
        return point_cloud, pressures, wss, vel
    ...
    def sample_point_cloud_with_pressure(self, mesh, n_points=5000):
        ...
        sampled_wss = mesh.point_data['wallShearStress'][indices]

        sampled_vel = mesh.point_data['U'][indices]

        return pv.PolyData(sampled_points), sampled_pressures, sampled_wss, sampled_vel
    ...
    def __getitem__(self, idx):
        ...
        if os.path.exists(cache_path):
            ...
            point_cloud, pressures, wss, vel = self._load_from_cache(cache_path)
        else:
            if self.preprocess:
                ...
                point_cloud, pressures, wss, vel = self.sample_point_cloud_with_pressure(mesh, self.num_points)

                # Cache the sampled data to a new file
                self._save_to_cache(cache_path, point_cloud, pressures, wss, vel)
            else:
        ...
        wss_tensor = torch.tensor(wss[np.newaxis, :], dtype=torch.float32)
        vel_tensor = torch.tensor(vel[np.newaxis, :], dtype=torch.float32)

        return point_cloud_tensor, pressures_tensor, wss_tensor, vel_tensor
...
def get_Volume_dataloaders(dataset_path: str, subset_dir: str, num_points: int, batch_size: int,
