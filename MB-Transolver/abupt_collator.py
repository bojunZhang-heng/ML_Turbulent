...
def to_prefix_and_postfix(item: str) -> tuple[str, str]:
    """将 item 拆分为前缀和后缀"""
    parts = item.split("_")
    if len(parts) >= 2:
...
def to_prefix_midfix_postfix(item: str) -> tuple[str, str, str]:
    """将 item 拆分为前缀、中间和后缀"""
    parts = item.split("_")
    if len(parts) == 3:
...
#                ),
                 AnchorPointSamplingPreprocessor(
                    ...
                    to_prefix_midfix_postfix=to_prefix_midfix_postfix,  # 使用定义的函数
                    seed=None if seed is None else seed + 3,
                 ),
